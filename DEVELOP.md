...
This document provides guidance for developers working on llm-coreml.
...
# Test coverage
uv run pytest --cov=src/llm_coreml --cov-report=term
```
...
```
llm-coreml/
├── src/llm_coreml/      # Package source code
...
Integration tests exercise the real CoreML inference pipeline (load model, tokenize, predict, decode) using a tiny GPT-2 model. They require macOS and a one-time model conversion step.

**Setup** (one-time):

```bash
# Convert tiny-gpt2 to CoreML (~5MB)
# Uses an isolated env with pinned deps (torch, transformers v4, coremltools)
just convert-test-model
```
...
Integration tests are marked with `@pytest.mark.integration` and auto-skip if the model hasn't been converted or the platform is not macOS. CI excludes them by default.

## Model Conversion

`scripts/convert_model.py` converts HuggingFace causal LMs to CoreML `.mlpackage` format using `coremltools`. Key details:

- **Format**: Uses `neuralnetwork` (not `mlprogram`). The `mlprogram` format segfaults on `predict()` with GPT-2 models due to known coremltools issues (#1960, #2166). The `neuralnetwork` format uses float32 by default and handles dynamic sequence lengths without crashes.
- **Dynamic shapes**: Uses `RangeDim` for flexible sequence lengths (1 to `--max-seq-len`, default 512).
- **Wrapper class**: GPT-2's `forward()` signature has `past_key_values` before `attention_mask`, so a `Wrapper` maps positional args to keyword args for correct tracing.
- **Isolated environment**: Conversion runs via `uv run --isolated` with pinned deps (Python 3.13, torch, transformers <4.46, coremltools >=8.0) to avoid version conflicts with the main project.

The inference engine (`src/llm_coreml/inference.py`) loads models with `compute_units=CPU_ONLY` for consistent float32 execution.
...
# Place in src/llm_coreml/py.typed (marker only)
# or src/llm_coreml/*.pyi (stub files)
```
